...
public class TerminalViewController implements ViewController {
    ...
    @Override
    public String presentTextSelectionListWithPrompt(List<String> options) {
        if (options == null || options.isEmpty()) {
            System.out.print("Enter text: ");
            return consumeTextInput().orElse("");
        }
        for (int i = 0; i < options.size(); i++) {
            System.out.println((i + 1) + ") " + options.get(i));
        }
        System.out.print("Choose an option number or type custom text: ");
